...
(function () {
  ...
  function renderFees() {
    ...
    // 医保报销比例：只取最近一次门诊，分母 = 医保范围内金额（金额 − 个人自费）
    var lr = f.latest_outpatient_reimburse || null;
    var reimbCard = "";
    if (lr && lr["医保报销比例"] != null) {
      reimbCard = '<div><div class="k">医保报销比例</div><div class="mid">' + lr["医保报销比例"] + '%</div>'
        + '<div style="font-size:11px;color:rgba(255,255,255,.62);margin-top:3px">'
        + esc(lr["日期"]) + ' 门诊 · 统筹 ' + money(lr["医保统筹"]) + ' ÷ 医保内 ' + money(lr["医保内金额"])
        + '</div></div>';
    }
    var hero = $("fees-hero");
    if (hero) {
      hero.innerHTML = '<div class="row">'
        ...
        + '</div>'
        + '<div class="note">统计区间 2026-07-02 至 2026-09-17　｜　医保类型：职工基本医疗保险　｜　含大病医保报销</div>';
    }
    var cnt = $("fees-count");
